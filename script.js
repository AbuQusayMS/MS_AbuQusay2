...
/* =========================================================
   ملحق الترقيات (Append-only) — ألصقه أسفل ملفك كما هو
   ========================================================= */
(function () {
    /* ————— أدوات صغيرة ————— */
    ...
    /* ————— تغليف init لحقن عناصر واجهة وتحميل الحالة ————— */
    const _init = QuizGame.prototype.init;
    QuizGame.prototype.init = async function() {
        await _init.call(this);
        try { this._enh_installOnce?.(); } catch(_) {}
    };

    /* ————— تبديل الصوت: نلف playSound ليتبع حالة الكتم ————— */
    const _play = QuizGame.prototype.playSound;
    QuizGame.prototype.playSound = function(name){
        if (this._muted) return;
        return _play.call(this, name);
    };

    /* ————— مؤثرات الكونفيتي ————— */
    QuizGame.prototype._ensureConfettiCanvas = function(){
    ...
    QuizGame.prototype._confetti = function(duration=900){
        const ctx = this._ensureConfettiCanvas();
        const W = ctx.canvas.width, H = ctx.canvas.height;
        const parts = Array.from({length: 120}, ()=>({
            x: Math.random()*W, y: -20 - Math.random()*80,
            r: 4+Math.random()*6, s: 2+Math.random()*4,
        ...
        const draw = (t)=>{
            ...
            parts.forEach(p=>{
                p.y += p.s; p.x += Math.sin(p.a+=0.05)*1.2;
                ctx.fillStyle = p.col;
                ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill();
            });
            if (performance.now() - (t - dt) < duration) requestAnimationFrame(draw);
    ...
    /* ————— سباركلان (SVG صغير) ————— */
    QuizGame.prototype._renderSparkline = function(values){
        values = (values||[]).slice(-10);
        if (!values.length) return '';
        const w=240,h=42,pad=4;
        const min=Math.min(...values), max=Math.max(...values);
        const norm = v => (max===min)? h/2 : (h - pad - ((v-min)/(max-min))*(h-2*pad));
        ...
        return `
            <div class="sparkline-wrap">
                <svg class="sparkline" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none">
                    ...
                    <circle cx="${pad+(values.length-1)*step}" cy="${norm(last)}" r="3"></circle>
                </svg>
                <div>أحدث ١٠ نتائج</div>
            </div>`;
    };

    /* ————— إنجازات خفيفة ————— */
    QuizGame.prototype._awardBadge = function(key, label){
        try{
            const k = `badges:${this.getOrSetDeviceId()}`;
            const set = new Set(JSON.parse(localStorage.getItem(k)||'[]'));
            if (!set.has(key)){
                set.add(key);
                localStorage.setItem(k, JSON.stringify(Array.from(set).slice(-50)));
                this.showToast(`🏅 إنجاز: ${label}`, 'success');
                this.playSound('coin');
            }
        }catch(_){}
    };
    ...
    /* ————— XP بسيط ————— */
    QuizGame.prototype._addXP = function(amount=0){
        try{
            const k = `xp:${this.getOrSetDeviceId()}`;
    ...
    QuizGame.prototype._getXP = function(){
        try{ return Number(localStorage.getItem(`xp:${this.getOrSetDeviceId()}`)||0); }catch(_){ return 0; }
    };
    QuizGame.prototype._renderXPBadge = function(){
        const host = qs('.player-details');
        if (!host) return;
        let b = qs('#xpBadge', host);
        if (!b){ b = document.createElement('span'); b.id='xpBadge'; b.style.cssText='font-size:.8rem;opacity:.9;margin-top:.1rem'; host.appendChild(b); }
        b.textContent = `XP: ${this.formatNumber(this._getXP())}`;
    };

    /* ————— زر “لعبة سريعة” وزر الكتم وزر التركيز ————— */
    QuizGame.prototype._enh_installOnce = function(){
        if (this.__enhanced) return; this.__enhanced = true;

        // زر لعبة سريعة
        const startBox = qs('#startScreen .button-group');
        if (startBox && !qs('#quickPlayBtn', startBox)){
            const btn = document.createElement('button');
            btn.id = 'quickPlayBtn';
            btn.className = 'btn-secondary';
            btn.textContent = '🎲 لعبة سريعة (5 أسئلة)';
            btn.setAttribute('data-action','__quickPlay');
            startBox.appendChild(btn);
        }

        // أزرار علويّة: كتم الصوت + وضع التركيز + الاهتزاز
        const topBar = qs('.top-bar');
        if (topBar){
            const mk = (txt,action,title) => {
                const b=document.createElement('button');
                b.className='theme-toggle-btn'; b.textContent=txt; b.title=title||'';
                b.setAttribute('data-action', action); return b;
            };
            if (!qs('[data-action="toggleMute"]', topBar)){
                topBar.appendChild(mk('🔊','toggleMute','كتم/تشغيل الصوت'));
            }
            if (!qs('[data-action="toggleFocus"]', topBar)){
                topBar.appendChild(mk('🧘','toggleFocus','وضع التركيز'));
            }
            if (!qs('[data-action="toggleVibration"]', topBar)){
                topBar.appendChild(mk('📳','toggleVibration','تفعيل/إيقاف الاهتزاز'));
            }
        }

        // حالات أوليّة
        try {
            this._muted = localStorage.getItem('quiz:muted')==='1';
            this._vibrate = localStorage.getItem('quiz:vibrate')!=='0';
            if (this._muted && qs('[data-action="toggleMute"]')) qs('[data-action="toggleMute"]').textContent='🔇';
        } catch(_){}

        // ربط الأفعال الجديدة
        const _bind = this.bindEventListeners.bind(this);
        this.bindEventListeners = function(){
            ...
            document.body.addEventListener('click', (e)=>{
                ...
                if (a==='__quickPlay'){
                    if (!this.guardAction(t,'__quickPlay')) return;
                    this._startQuickPlay();
                } else if (a==='toggleMute'){
                    this._muted = !this._muted;
                    try{ localStorage.setItem('quiz:muted', this._muted?'1':'0'); }catch(_){}
                    t.textContent = this._muted ? '🔇' : '🔊';
                    this.showToast(this._muted?'تم كتم الصوت':'تم تشغيل الصوت','info');
                } else if (a==='toggleFocus'){
                    const on = document.body.dataset.focus==='on';
                    document.body.dataset.focus = on ? 'off' : 'on';
                    this.showToast(on?'تم إيقاف وضع التركيز':'تم تفعيل وضع التركيز','info');
                } else if (a==='toggleVibration'){
                    this._vibrate = !this._vibrate;
                    try{ localStorage.setItem('quiz:vibrate', this._vibrate?'1':'0'); }catch(_){}
                    this.showToast(this._vibrate?'تم تفعيل الاهتزاز':'تم إيقاف الاهتزاز','info');
                }
            }, {capture:true});
        };
    ...
    /* ————— لعبة سريعة: 5 أسئلة عشوائية من كل المستويات ————— */
    QuizGame.prototype._startQuickPlay = function(){
        try{
            this.cleanupSession().then(()=>{
                this.setupInitialGameState();
                this.gameState.quickPlay = true;
                // اجلب من كل المستويات وأخذ 5 عشوائي
                const all = this.getLevelQuestions('easy')
                    .concat(this.getLevelQuestions('medium'))
                    .concat(this.getLevelQuestions('hard'))
                    .concat(this.getLevelQuestions('impossible'));
                this.shuffleArray(all);
                this.gameState.shuffledQuestions = all.slice(0,5);
                this.gameState.level = 0; // لواجهة المؤشر
                document.body.dataset.level = 'easy';
                this.updateScore(this.config.STARTING_SCORE, true);
                this.setupGameUI();
                this.showScreen('game');
                this.playSound('start');
                this.fetchQuestion();
                this.showToast('🎲 بدأ وضع اللعبة السريعة: 5 أسئلة!', 'info');
            });
        }catch(_){}
    };

    /* ————— سلسلة صحيحة + بونص ————— */
    const _check = QuizGame.prototype.checkAnswer;
    QuizGame.prototype.checkAnswer = function(selectedButton=null){
        // نحدد الصحيح مسبقاً للفلاش/كونفيتي/ستريك
        const wasCorrect = !!(selectedButton && selectedButton.dataset && selectedButton.dataset.correct==='true');
        if (wasCorrect){
            selectedButton.classList.add('flash');
            this._streak = (this._streak||0) + 1;
            // بونص عند 3/5
            if (this._streak===3){
                this.updateScore(this.gameState.currentScore + 50);
                this.showToast('🔥 سلسلة 3! +50', 'success');
                this.playSound('coin'); this._confetti(700);
            } else if (this._streak===5){
                this.updateScore(this.gameState.currentScore + 100);
                this.showToast('⚡ سلسلة 5! +100', 'success');
                this.playSound('coin'); this._confetti(900);
                this._awardBadge('streak5','سلسلة 5 إجابات متتالية');
            }
        } else {
            this._streak = 0;
            if (this._vibrate && navigator.vibrate) { try{ navigator.vibrate(80); }catch(_){ } }
        }
        const ret = _check.call(this, selectedButton);
        if (wasCorrect){ this._confetti(420); }
        return ret;
    };

    /* ————— مؤثر “ذعر” للمؤقت + تدرّج ديناميكي ————— */
    const _startTimer = QuizGame.prototype.startTimer;
    QuizGame.prototype.startTimer = function(){
        _startTimer.call(this);
        try{
            const box = qs('.timer-container');
            const bar = qs('.timer-bar');
            const label = qs('.timer-text');
            if (!box || !bar || !label) return;

            // نعيد تشغيل مراقبة كل ثانية
            if (this.__timerEnh) clearInterval(this.__timerEnh);
            const total = this.config.QUESTION_TIME;
            this.__timerEnh = setInterval(()=>{
                const left = Number(label.textContent||'0');
                // حالات UI
                box.classList.toggle('low', left<=10 && left>5);
                box.classList.toggle('panic', left<=5);
                // تدرج لوني ناعم من أخضر -> أحمر
                const p = clamp(left/total,0,1);
                const hue = Math.round(120*p); // 120 (أخضر) إلى 0 (أحمر)
                bar.style.background = `linear-gradient(90deg, hsl(${hue} 80% 45%), var(--gold-color))`;
            }, 250);
            this.cleanupQueue.push({type:'interval', id:this.__timerEnh});
        }catch(_){}
    };

    /* ————— تحديث عناوين المساعدات (تلميحات) ————— */
    const _updateUI = QuizGame.prototype.updateGameStatsUI;
    QuizGame.prototype.updateGameStatsUI = function(){
        _updateUI.call(this);
        try{
            qsa('.helper-btn').forEach(btn=>{
                const type = btn.dataset.type;
                if (type==='skipQuestion'){
                    btn.title = 'تخطي السؤال (مجاني)';
                } else if (type==='fiftyFifty'){
                    btn.title = this.gameState.helpersUsed.fiftyFifty ? 'مستخدمة' : 'يحذف خيارين خاطئين (مرة واحدة)';
                } else if (type==='freezeTime'){
                    btn.title = this.gameState.helpersUsed.freezeTime ? 'مستخدمة' : 'إيقاف الوقت 10 ثوانٍ (مرة واحدة)';
                }
            });
        }catch(_){}
    };

    /* ————— إظهار السباركلان + حفظ السجل + إنجازات + XP عند النهاية ————— */
    const _displayFinal = QuizGame.prototype.displayFinalStats;
    QuizGame.prototype.displayFinalStats = function(stats){
        _displayFinal.call(this, stats);
        try{
            // حفظ آخر 10 نقاط
            const k = `history:${this.getOrSetDeviceId()}`;
            let arr = []; try{ arr = JSON.parse(localStorage.getItem(k)||'[]'); }catch(_){}
            arr.push(Number(stats.score||0)); arr = arr.slice(-10);
            try{ localStorage.setItem(k, JSON.stringify(arr)); }catch(_){}
            // حقن السباركلان
            const host = qs('#endScreen .final-stats');
            if (host){ host.insertAdjacentHTML('beforeend', this._renderSparkline(arr)); }
            // إنجازات بسيطة
            if (String(stats.level).includes('مستحيل') || String(stats.level).toLowerCase().includes('impossible')){
                this._awardBadge('reach-impossible','وصلت إلى المستوى المستحيل');
            }
            if (stats.completed_all){ this._awardBadge('finish-all','أنهيت جميع المستويات'); }
            if (Number(stats.accuracy||0) >= 90){ this._awardBadge('acc90','دقة 90%+'); }
            // XP: +1 لكل إجابة صحيحة + 0.1 لكل نقطة/100
            const xpGain = Math.round((Number(stats.correct_answers||0)) + (Number(stats.score||0)/100)*0.1);
            this._addXP(xpGain);
        }catch(_){}
    };

    /* ————— عند البدء: حدّث شارة XP دوماً ————— */
    const _setupUI = QuizGame.prototype.setupGameUI;
    QuizGame.prototype.setupGameUI = function(){
        _setupUI.call(this);
        try{ this._renderXPBadge(); }catch(_){}
    };
})();
